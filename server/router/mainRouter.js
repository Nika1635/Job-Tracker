...
appRouter.get('/jobs', async (req, res) => {
    const jobs = await sql`SELECT * FROM jobs;`
    res.json(jobs)
})

appRouter.post("/jobs", async (req, res) => {
...
appRouter.get("/jobs/:id", async (req, res) => {
    const { id } = req.params
    const searchById = await sql `SELECT * FROM jobs WHERE id = ${id};`
    res.json(searchById)
})

appRouter.delete("/jobs/:id", async (req, res) => {
    const { id } = req.params
    const removeData = await sql `DELETE FROM jobs WHERE id = ${id};`
    res.json(removeData)
})

appRouter.patch("/jobs/:id", async (req, res) => {
    const { id } = req.params
